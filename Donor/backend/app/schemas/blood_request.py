...
class BloodRequestBase(BaseModel):
    ...
    request_status: str = Field("Pending", description="Allowed: Pending, Fulfilled, Cancelled")
    ...
    def validate_request_status(cls, v: str) -> str:
        ...
        if v not in allowed:
            raise ValueError("Request status must be one of Pending, Fulfilled, or Cancelled.")
        return v

    @field_validator("contact_number")
    @classmethod
    def validate_bangladesh_phone(cls, v: str) -> str:
        # Bangladesh phone numbers: optional +88/88, then 01 and 9 digits. e.g. +8801712345678, 01812345678
...
# Model to represent response output
